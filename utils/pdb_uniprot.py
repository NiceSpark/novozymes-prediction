...
def get_uniprot_infos(pdb_ids=[], uniprot_ids=[],
    ...
    if pdb_uniprot_mapping_df is None:
        ...
        pdb_uniprot_mapping_df = pd.read_csv(
            "./data/main_dataset/pdb_uniprot_mapping.csv")
    ...
    if pdb_ids:
        pdb_uniprot_ids = pdb_uniprot_mapping_df[pdb_uniprot_mapping_df["PDB_wild"].isin(
            pdb_ids)]

        mapping = list(map(
            lambda x: [x["PDB_wild"], x["uniprot"]], pdb_uniprot_ids.T.to_dict().values()))
        uniprot_infos = uniprot_infos_df[uniprot_infos_df["uniprot"].isin(
            [x[1] for x in mapping])]
        for [pdb, uniprot] in mapping:
            info_lst = uniprot_infos.loc[uniprot_infos.uniprot.eq(
                uniprot)].to_dict("records")
...
# ## look at neighbors to find right mutation position, used with neighbors=1
...
def correct_mutation_position(wild_aa: str, mutation_position: int, sequence: str,
    ...
    """
    position = mutation_position+chain_start+position_offset
...
def coherent_uniprot(mutation_code: str, pdb_ids=[], uniprot_ids=[],
    ...
    if pdb_ids:
        ...
        if uniprot_ids:
            ...
            unique_uniprot_ids = list(
                set([v.get("uniprot") for k, v in uniprot_infos.items()]))
        ...
        for pdb, infos in uniprot_infos.items():
            ...
            corrected_code = correct_mutation_code(
                mutation_code, infos["sequence"], infos["chain_start"])
            if (corrected_code
